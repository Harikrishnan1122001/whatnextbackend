...
// ── Base HTML template — Red & White WhatNext theme ────────────
const baseTemplate = (title, bodyHtml) => `
<!DOCTYPE html>
<html>
<head>
  ...
  <style>
    body { font-family: 'Segoe UI', Arial, sans-serif; background: #f4f6f9; margin: 0; padding: 20px; }
    .container { max-width: 600px; margin: 0 auto; background: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 24px rgba(232,17,59,0.10); }
    .header { background: linear-gradient(135deg, #e8113b 0%, #ff4d6d 100%); padding: 32px 40px; text-align: center; }
    .header h1 { color: #fff; margin: 0; font-size: 24px; letter-spacing: -0.5px; }
    .header p  { color: rgba(255,255,255,0.85); margin: 8px 0 0; font-size: 14px; }
    .body { padding: 32px 40px; color: #374151; line-height: 1.7; background: #ffffff; }
    .body h2 { font-size: 20px; color: #111827; margin-top: 0; }
    .btn { display: inline-block; padding: 14px 32px; background: linear-gradient(135deg,#e8113b,#ff4d6d); color: #fff !important; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 15px; margin: 16px 0; }
    .info-box { background: #fff5f6; border-left: 4px solid #e8113b; border-radius: 4px; padding: 16px 20px; margin: 20px 0; }
    .info-box p { margin: 4px 0; font-size: 14px; }
    .info-box strong { color: #111827; }
    .footer { background: #fafafa; padding: 20px 40px; text-align: center; border-top: 1px solid #f0d0d5; }
    .footer p { color: #9ca3af; font-size: 12px; margin: 4px 0; }
    .divider { height: 1px; background: #f0d0d5; margin: 24px 0; }
  </style>
</head>
<body>
  <div class="container">
...
// ── 3. Live Class Meeting Details (ID + Password) ───────────────
// Sent immediately on registration if meeting details already exist,
// and again before the class starts (bulk send by the admin).
const sendLiveClassUrl = async (user, liveClass) => {
  const formattedDate = new Date(liveClass.scheduledAt).toLocaleString('en-IN', {
    weekday: 'long', year: 'numeric', month: 'long',
    day: 'numeric', hour: '2-digit', minute: '2-digit'
  ...
  const html = baseTemplate('Your Live Class Link', `
    <h2>You're Registered — Here Are Your Meeting Details! 🔴</h2>
    <p>Hi <strong>${user.name}</strong>, here are the details for your upcoming live session.</p>
    <div class="info-box">
      <p><strong>📚 Session:</strong> ${liveClass.title}</p>
      <p><strong>📅 Date & Time:</strong> ${formattedDate}</p>
    ...
    <p style="color:#6b7280;font-size:13px;">If the button doesn't work, copy this link:<br>
      <a href="${liveClass.meetingUrl}" style="color:#e8113b;">${liveClass.meetingUrl}</a>
    </p>` : `
    <p style="color:#6b7280;font-size:13px;">The join link will be sent to you closer to the class start time.</p>`}
  `);

  return transporter.sendMail({
    from: `${PLATFORM_NAME} <${FROM}>`,
    to: user.email,
    subject: `🔴 Live Class Details: ${liveClass.title}`,
    html
  });
};

// ── 4. Live Class Registration Confirmation (no meeting details yet) ──
const sendRegistrationConfirmation = async (user, liveClass) => {
  const formattedDate = new Date(liveClass.scheduledAt).toLocaleString('en-IN', {
    weekday: 'long', year: 'numeric', month: 'long',
    day: 'numeric', hour: '2-digit', minute: '2-digit'
  ...
  const html = baseTemplate('Registration Confirmed', `
    ...
    <p>You'll receive the meeting ID & password <strong>${liveClass.urlSendMinutesBefore} minutes</strong> before the class starts.</p>
    <div style="text-align:center;">
      <a class="btn" href="${CLIENT_URL}/dashboard/live-classes">View My Classes</a>
    </div>
  `);
