...
//   const courses = await Course.find(query)
//     .populate('instructor', 'name avatar')
//     .select('-videos.videoUrl')          // hide video URLs in list view
//     .sort({ createdAt: -1 })
//     .skip(skip)
//     .limit(Number(limit));
...
//   // ── Mask video URLs for non-purchasers ────────────────────
//   const courseData = course.toObject();
//   courseData.videos = courseData.videos.map(v => ({
//     ...v,
//     // Only expose the real URL to admins, purchasers, or preview videos
//     videoUrl: (purchased || v.isPreview) ? v.videoUrl : null,
//     locked: !purchased && !v.isPreview,
//   }));

//   // ── IMPORTANT: always return { course, purchased } together ─
//   // The frontend reads:  d.data.course  and  d.data.purchased
//   return sendSuccess(res, { course: courseData, purchased });
// });
...
//     updates.isFree = updates.isFree === 'true' || updates.isFree === true;
...
//   // Return the new flag so the frontend can update without an extra fetch
//   return sendSuccess(
//     res,
//     { isPublished: course.isPublished, publishedAt: course.publishedAt },
//     `Course ${course.isPublished ? 'published' : 'unpublished'}`
//   );
// });
...
//   const savedVideo = course.videos[course.videos.length - 1];
//   return sendSuccess(res, { video: savedVideo }, 'Video uploaded successfully', 201);
// });
...
//   const savedVideo = course.videos[course.videos.length - 1];
//   return sendSuccess(res, { video: savedVideo }, 'Video URL added successfully', 201);
// });
...
exports.getCourse = asyncHandler(async (req, res) => {
  ...
  // ── Determine purchase / enrolment status ──────────────────
  let purchased = false;
  if (req.user) {
    const user = await User.findById(req.user.id);
    purchased = user.hasPurchasedCourse(course._id);
  ...
  // Always return { course, purchased } — frontend reads d.data.course & d.data.purchased
  return sendSuccess(res, { course: courseData, purchased });
});

// POST /api/courses/:id/enrol  ← FREE enrolment (no payment)
exports.enrolFree = asyncHandler(async (req, res) => {
  const course = await Course.findById(req.params.id);
  if (!course || !course.isPublished) throw new AppError('Course not found', 404);
  if (!course.isFree) throw new AppError('This course is not free', 400);
  ...
  return sendSuccess(res, { purchased: true }, 'Enrolled successfully!');
});
