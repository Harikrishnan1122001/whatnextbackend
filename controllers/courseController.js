...
// Free-course enrollment. The frontend "Enrol Free" button must call this —
// previously it only showed a toast and never hit the backend, so
// hasPurchasedCourse() always returned false and videos stayed locked forever.
exports.enrollFreeCourse = asyncHandler(async (req, res) => {
  const course = await Course.findById(req.params.id);
  if (!course || !course.isPublished) throw new AppError('Course not found', 404);
  if (!course.isFree) throw new AppError('This course is not free. Use the payment flow instead.', 400);

  const user = await User.findById(req.user.id);
  if (!user) throw new AppError('User not found', 404);

  if (user.hasPurchasedCourse(course._id)) {
    return sendSuccess(res, { purchased: true, alreadyEnrolled: true }, 'You are already enrolled in this course');
  }
  ...
  return sendSuccess(res, { purchased: true, alreadyEnrolled: false }, 'Enrolled successfully');
});
